...
function learn_weights(weight_learner::WeightLearner, Ps_train, Xs_train, Xs_test, α, multiple_testing_method) end

struct UnitWeightLearner <: WeightLearner end

function learn_weights(::UnitWeightLearner, Ps_train, Xs_train, Xs_test, α, multiple_testing_method)
	ones(size(Xs_test,1)), nothing
end

struct IHWResult{I<:IHW,
	             XT,
				 P,
...
function fit(ihw::IHW, Ps, Xs)
    ...
    for (train_idx, test_idx) in kf
        ...
        tmp_wts, tmp_fit = learn_weights(weight_learner, view(Ps, train_idx), Xs_train_view,
		                                         Xs_test_view, α,  multiple_testing_method)
