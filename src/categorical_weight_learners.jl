...
function learn_weights(gbh::GBHLearner, Ps_train,
	...
	function gbh_wt(Ps)
		pi0 = estimate_pi0(Ps, gbh.pi0estimator)
		(1-pi0)/pi0
	end
...
function learn_weights(::GrenanderLearner, Ps_train,
	                                    Xs_train::AbstractVector{<:CategoricalValue},
										Xs_test::AbstractVector{<:CategoricalValue},
										α, ::BenjaminiHochberg)
	...
	α = 0.1
	λ_idx = findfirst(λ -> α ∈ lagrange_balance(λ,train_grenander,test_ms,α,BenjaminiHochberg()), all_λs)
	λ_opt = all_λs[λ_idx]
	...
	lin_Fdr = lagrange_balance(λ_opt, train_grenander, test_ms, α, BenjaminiHochberg(); linearized=true)
